...
function buildTitleIData() {
  ...
  return [
    ...
    { metric: 'Engagement %',      max: 100, titleI: avgField(t1, 'engagement'),   nonTitleI: avgField(n1, 'engagement') },
    { metric: 'Session (min)',     max: 30,  titleI: avgField(t1, 'avgSession'),   nonTitleI: avgField(n1, 'avgSession') },
    { metric: 'Streak Rate %',     max: 100, titleI: avgField(t1, 'streakPct'),    nonTitleI: avgField(n1, 'streakPct') },
    { metric: 'Lexile Growth (L)', max: 120, titleI: avgField(t1, 'lexileGrowth'), nonTitleI: avgField(n1, 'lexileGrowth') },
  ]
}
...
const EQUITY_COLUMNS = [
  ...
  { key: 'lexileGrowth', label: 'Lexile Δ', align: 'right', render: v => v > 0 ? `+${v}L` : `${v}L` },
  {
    key: 'frl',
    label: 'FRL %',
    align: 'center',
  ...
  },
  {
    ...
    align: 'center',
    render: v => v
      ? <Pill color="#1D4ED8" variant="soft" size="sm">Title I</Pill>
  ...
  return (
    <div className="rc-page" style={{ '--rc-accent': ACCENT }}>
      ...
      <div className="rc-stats-row">
        ...
        <StatCard value="−17 pts" label="Title I RMI Gap" footer="vs. non-Title I schools" color={T1_COLOR} />
        <StatCard value="42%" label="Avg FRL Rate" footer="Free/reduced lunch, district" />
        <StatCard value="−23 pts" label="FRL Engagement Gap" footer="High FRL vs. low FRL schools" color={T1_COLOR} />
      </div>
        ...
        >
          <BarList
            groups={titleIData.map(row => ({
              ...
              labelColor: '#475569',
              items: [
                { label: 'Title I',     labelColor: T1_COLOR, value: row.titleI,    max: row.max, color: T1_COLOR, valueLabel: String(row.titleI) },
                { label: 'Non-Title I', labelColor: N1_COLOR, value: row.nonTitleI, max: row.max, color: N1_COLOR, valueLabel: String(row.nonTitleI) },
              ],
            }))}
          />
        </ChartCard>

...
import { ChartLegend, NIVO_THEME, AXIS_BOTTOM, AXIS_LEFT } from './charts'
...
const STW_COLUMNS = [
  ...
  { key: 'motivation', label: 'M', align: 'center', render: (_, r) => <ScoreCell value={r.health.motivation} /> },
  { key: 'integrity',  label: 'I', align: 'center', render: (_, r) => <ScoreCell value={r.health.integrity} /> },
  { key: 'habits',     label: 'H', align: 'center', render: (_, r) => <ScoreCell value={r.health.habits} /> },
  { key: 'skills',     label: 'S', align: 'center', render: (_, r) => <ScoreCell value={r.health.skills} /> },
  {
    key: 'action',
    label: '',
    align: 'right',
    render: () => <button className="rc-card-drill" disabled>View →</button>,
  },
]
    ...
    <div className="rc-page">
      ...
      <div className="sv-grid">
        ...
        <ChartCard
          ...
          accent="#0DA7BC"
          bodyPad="padded"
          action={<button className="rc-card-drill" onClick={() => onNavigate('motivation')}>View details →</button>}
          footer={
            <ChartLegend
              items={[
        ...
        >
          <TrendChart
            ...
            series={[
              ...
              ...SCHOOLS.map(s => ({ key: s.id, name: s.name, color: s.color, dashed: true, fillOpacity: 0 })),
            ]}
          />
        </ChartCard>
        ...
        <ChartCard
          ...
          accent="#16A97A"
          bodyPad="padded"
          action={<button className="rc-card-drill" onClick={() => onNavigate('habits')}>View details →</button>}
          footer={<ChartLegend items={[{ color: '#16A97A', label: 'District avg session length' }]} />}
        >
          <div className="dash-inline-stats">
            <div className="dash-inline-stat">
              <span className="dash-inline-val">20<span className="dash-inline-unit">min</span></span>
              <span className="dash-inline-lbl">avg session</span>
            </div>
            <div className="dash-inline-stat">
              <span className="dash-inline-val">41<span className="dash-inline-unit">%</span></span>
          ...
          <TrendChart
            ...
            series={[
              { key: 'district', name: 'District avg', color: '#16A97A' },
            ]}
          />
        </ChartCard>
        ...
        <ChartCard
          ...
          accent="#1D4ED8"
          bodyPad="padded"
          action={<button className="rc-card-drill" onClick={() => onNavigate('integrity')}>View details →</button>}
          footer={
            <ChartLegend
              items={[
        ...
        <ChartCard
          ...
          accent="#7C3AED"
          bodyPad="padded"
          action={<button className="rc-card-drill" onClick={() => onNavigate('skills')}>View details →</button>}
          footer={
            <ChartLegend
              items={[
        ...
        >
          <div style={{ flex: 1, minHeight: 240 }}>
            <ResponsiveScatterPlot
              ...
              axisBottom={{ ...AXIS_BOTTOM, legend: 'Avg books/month', legendOffset: 44, legendPosition: 'middle', tickValues: 5 }}
              axisLeft={{ ...AXIS_LEFT, format: v => `${v}L`, legend: 'Lexile growth', legendOffset: -58, legendPosition: 'middle', tickValues: 5 }}
              enableGridX={false}
              tooltip={({ node }) => (
                <div className="sdb-tooltip" style={{ '--tip-accent': node.serieId === 'Above expected' ? '#0DA7BC' : '#E8866A' }}>
                  <div className="sdb-tooltip-header">{node.data.school}</div>
                  <div className="sdb-tooltip-series" style={{ '--series-color': node.serieId === 'Above expected' ? '#0DA7BC' : '#E8866A' }}>

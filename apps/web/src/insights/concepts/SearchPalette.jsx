...
export function SearchPalette({ onMeterChange, onOpenDetail, onCustomize, visibleTiles }) {
  ...
  const matches = useMemo(() => {
    ...
    return ALL_ITEMS
      .filter(i => visibleTiles.has(i.id))
    ...
    <div className="ins-page">
      ...
      <div className="ins-palette">
        ...
        {matches.length > 0 ? (
          <div className="ins-palette-matches">
            {matches.map(m => (
              <button key={m.id} type="button" className="ins-palette-match" onClick={() => open(m.id)}>
                <span>{m.label}</span>
                <span className="ins-palette-match-kind">{m.kind === 'panel' ? 'Panel' : 'Metric'}</span>
              </button>
            ))}
          </div>
        ) : (
          suggestions.length > 0 && (
            <div className="ins-palette-suggest">
              ...
              {suggestions.map(s => (
                <button key={s.id} type="button" className="ins-palette-chip" onClick={() => open(s.id)}>
                  {s.label}
                </button>
              ))}
            </div>
      ...
      {openedIds.length === 0 ? (
        ...
        <>
          ...
          {openedIds.filter(id => ALL_ITEMS.find(i => i.id === id)?.kind === 'metric').length > 0 && (

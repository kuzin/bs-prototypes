...
export function CachedRefresh({ onMeterChange, onOpenDetail, onCustomize, visibleTiles }) {
  ...
  function refresh() {
    ...
    setTimeout(() => {
      const now = new Date()
      setSnapshotAt(now.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', timeZoneName: 'short' }))
      setRefreshing(false)
    }, 1400)
  }
    ...
    <div className="ins-page">
      ...
      <div className="ins-cache-banner ins-cache-banner--cached">
        <div className="ins-cache-banner-text">
          <strong>Showing cached snapshot</strong> — last refreshed at {snapshotAt}.
          Numbers update nightly. Hit Refresh for live data ({visibleTiles.size} tiles → 1 batched query).
        </div>
      ...
      {visibleTiles.has('top-badges') && (
        <PanelCard title="Top 2 Earned Badges" kind="badges" state={refreshing ? 'loading' : 'value'}>
          <TopBadgesRow badges={TOP_BADGES} />
        </PanelCard>
      )}
      {visibleTiles.has('ages') && (
        <PanelCard title="Ages (New Registrations)" kind="ages" state={refreshing ? 'loading' : 'value'}>
          <AgesChart ages={AGES} />
        </PanelCard>
      )}
    </div>

...
const ABOVE_FOLD_IDS = [
  'active-readers', 'new-registrations', 'challenge-completions', 'completed-activities',
  'rewards-redeemed', 'badges-earned', 'reviews-submitted', 'books-read', 'minutes-read',
]
  ...
  return (
    <div className="ins-page">
      ...
      <div className="ins-cache-banner ins-cache-banner--progressive">
        ...
        <span>Below-the-fold panels wait until you scroll near them — fewer concurrent queries.</span>
      </div>
      ...
      {visibleTiles.has('top-books') && (
        <div ref={el => { tileRefs.current['top-books'] = el }} data-tile-id="top-books">
          <PanelCard title="Top 12 Books Read" kind="books" state={loaded['top-books'] ? 'value' : 'loading'}>
      ...
      {visibleTiles.has('top-badges') && (
        <div ref={el => { tileRefs.current['top-badges'] = el }} data-tile-id="top-badges">
          <PanelCard title="Top 2 Earned Badges" kind="badges" state={loaded['top-badges'] ? 'value' : 'loading'}>
      ...
      {visibleTiles.has('ages') && (
        <div ref={el => { tileRefs.current['ages'] = el }} data-tile-id="ages">
          <PanelCard title="Ages (New Registrations)" kind="ages" state={loaded['ages'] ? 'value' : 'loading'}>
            <AgesChart ages={AGES} />
          </PanelCard>
        </div>
      )}

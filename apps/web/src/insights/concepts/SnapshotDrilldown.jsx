...
export function SnapshotDrilldown({ onMeterChange, onOpenDetail, onCustomize, visibleTiles }) {
  ...
  }, [queriesFired, headlineLoading, loadedCount, total, headlineQueryCount, visiblePanels.length, onMeterChange])
      ...
      {visibleMetrics.length > 0 && (
        <div className="ins-snapshot">
          <div className="ins-snapshot-tag">
            <span>Snapshot · {visibleMetrics.length} metric{visibleMetrics.length === 1 ? '' : 's'} in a single call</span>
          </div>

...
export function AISnapshot({ onMeterChange, onOpenDetail, onCustomize, visibleTiles }) {
  ...
  function Stat({ id, fmtFn = fmt }) {
    ...
    return (
      <button type="button" className="ins-ai-stat" onClick={() => onOpenDetail?.(id)} title={`Open ${m.label}`}>
        {fmtFn(m.value)}
      </button>
    )
  }
  ...
  return (
    <div className="ins-page">
      ...
      <div className="ins-cache-banner ins-cache-banner--ai">
        ...
        <span>A single query produces the paragraph — click any number to drill in. Panels load on demand.</span>
      </div>
      ...
      <div className={`ins-ai-card ins-ai-card--${loading ? 'loading' : 'loaded'}`}>
        ...
        {loading ? (
          ...
          <p className="ins-ai-text">
            This week, <Stat id="active-readers" /> readers were active in your library,
            with <Stat id="new-registrations" /> new readers joining a challenge.
            Together they logged <Stat id="books-read" /> books across{' '}
            <Stat id="minutes-read" /> minutes of reading time
      ...
      {has('top-books') && (
        <PanelCard title="Top 12 Books Read" kind="books" state={panelStateFor('top-books')} onLoad={() => loadPanel('top-books')}>
          <TopBooksGrid books={TOP_BOOKS} />
        </PanelCard>
      )}
      {has('top-badges') && (
        <PanelCard title="Top 2 Earned Badges" kind="badges" state={panelStateFor('top-badges')} onLoad={() => loadPanel('top-badges')}>
          <TopBadgesRow badges={TOP_BADGES} />
        </PanelCard>
      )}
      {has('ages') && (
        <PanelCard title="Ages (New Registrations)" kind="ages" state={panelStateFor('ages')} onLoad={() => loadPanel('ages')}>
          <AgesChart ages={AGES} />
        </PanelCard>
      )}
    </div>

...
export function FlaggedView({ sessions, onSelectSession, onApproveRequest, onViewProfile, groupBy, defaultFilters = {} }) {
  ...
  }, [defaultFilters.grade, defaultFilters.classFilter, defaultFilters.flagType, defaultFilters.status, defaultFilters.challenge])

  const flagged = sessions.filter(s => (s.type === 'flagged' || s.type === 'both') && (s.flags?.length ?? 0) > 0)
  ...
  const activeFilters = [
    ...
    ...(classFilter !== 'all' ? [{ key: 'class',     label: `Class: ${classFilter}`,           onClear: () => setClassFilter('all')     }] : []),
    ...(flagType    !== 'all' ? [{ key: 'flagType',  label: `Flag: ${FLAG_TYPE_LABELS[flagType] ?? flagType}`, onClear: () => setFlagType('all') }] : []),
    ...
    <div className="lv-shell">
      ...
      <div className="lv-filters">
        ...
        <ActiveFilters
          ...
          onClearAll={() => { setSearch(''); setGrade('all'); setClassFilter('all'); setFlagType('all'); setStatus('all'); setChallenge('all') }}
        />
      </div>
      {groupBy === 'reader'

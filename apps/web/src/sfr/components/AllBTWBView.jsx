...
export function AllBTWBView({ sessions, onSelectSession, onApproveRequest, groupBy, defaultFilters = {} }) {
  ...
  const filtered = sessions.filter(s => {
    const matchType =
      type === 'all'      ? true :
      type === 'approved' ? wasApproved(s) :
      type === 'flagged'  ? (s.type === 'flagged' || s.type === 'both') && !wasApproved(s) :
      type === 'engagement' ? (s.type === 'engagement' || s.type === 'both') && !wasApproved(s) :
      false
  ...
  const activeFilters = [
    ...
    ...(type   !== 'all' ? [{ key: 'type',     label: `Type: ${TYPE_LABELS[type]}`,           onClear: () => setType('all')     }] : []),
    ...(rating !== 'all' ? [{ key: 'rating',   label: `Engagement: ${RATING_LABELS[rating]}`, onClear: () => setRating('all')   }] : []),
    ...(status !== 'all' ? [{ key: 'status',   label: `Status: ${STATUS_LABELS[status]}`,     onClear: () => setStatus('all')   }] : []),
    ...
    <div className="lv-shell">
      ...
      <div className="lv-filters">
        ...
        <ActiveFilters
          ...
          onClearAll={() => { setSearch(''); setType('all'); setRating('all'); setStatus('all'); setPosFlags('all') }}
        />
      </div>
      {groupBy === 'reader'

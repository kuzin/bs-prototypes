...
export function EngagementView({ sessions, onSelectSession, onApproveRequest, onViewProfile, groupBy, defaultFilters = {} }) {
  ...
  }, [defaultFilters.rating, defaultFilters.posFlags, defaultFilters.classFilter, defaultFilters.grade, defaultFilters.challenge, defaultFilters.status, defaultFilters.hasFlags])
  ...
  const filtered = engagement.filter(s => {
    ...
    const matchPosFlags    = posFlags    === 'all' || (posFlags    === 'has' ? (s.positiveFlags?.length > 0) : (s.positiveFlags?.length === 0))
    ...
    const matchHasFlags    = hasFlags    === 'all' || (hasFlags === 'has' ? (s.flags?.length > 0) : (s.flags?.length === 0))
    const matchSearch      = !search || (
      s.student.name.toLowerCase().includes(search.toLowerCase()) ||
      s.book.title.toLowerCase().includes(search.toLowerCase())
    )
    return matchRating && matchPosFlags && matchClassFilter && matchGrade && matchChallenge && matchStatus && matchHasFlags && matchSearch
  })

  const RATING_LABELS = { green: 'Positive', yellow: 'Mixed', red: 'Disengaged' }
  const activeFilters = [
    ...(search         ? [{ key: 'search',      label: `"${search}"`,                                         onClear: () => setSearch('')           }] : []),
    ...(rating      !== 'all' ? [{ key: 'rating',      label: `Engagement: ${RATING_LABELS[rating]}`,         onClear: () => setRating('all')        }] : []),
    ...(posFlags    !== 'all' ? [{ key: 'posFlags',    label: posFlags    === 'has' ? 'Has positive flags' : 'No positive flags', onClear: () => setPosFlags('all')    }] : []),
    ...(classFilter !== 'all' ? [{ key: 'classFilter', label: `Class: ${classFilter}`,                         onClear: () => setClassFilter('all')   }] : []),
    ...(grade       !== 'all' ? [{ key: 'grade',       label: `Grade: ${grade}`,                               onClear: () => setGrade('all')         }] : []),
    ...(challenge   !== 'all' ? [{ key: 'challenge',   label: `Challenge: ${challenge}`,                       onClear: () => setChallenge('all')     }] : []),
    ...(status      !== 'all' ? [{ key: 'status',      label: status      === 'completed' ? 'Finished' : 'Unfinished', onClear: () => setStatus('all') }] : []),
    ...(hasFlags    !== 'all' ? [{ key: 'hasFlags',    label: hasFlags    === 'has' ? 'Has integrity flags' : 'No integrity flags', onClear: () => setHasFlags('all') }] : []),
  ]

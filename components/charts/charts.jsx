...
// The .sdb-tooltip styles still live in SchoolDashboard.css; the legend's own
// styles moved here so <ChartLegend> draws correctly wherever it is used.
          ...
          >
            ...
            {delta != null && (
              <div className={`sdb-tooltip-delta sdb-tooltip-delta--${cls}`}>
                ...
                <span>
                  {formatDelta ? formatDelta(delta) : delta > 0 ? `+${delta}` : delta} {deltaLabel}{' '}
                  {prev.month}
                </span>
              </div>
            )}
  ...
  return (
    <div className="sdb-tooltip" style={{ '--tip-accent': accent }}>
      ...
      >
        ...
        <span>
          {isAbove ? '+' : ''}
          {delta}L {isAbove ? 'above' : 'below'} target
        </span>
      </div>
    </div>
  )

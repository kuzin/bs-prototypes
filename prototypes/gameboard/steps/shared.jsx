...
export function Tip({ children }) {
  return (
    <Banner level="info" className="gb-tip-note">
      {children}
    </Banner>
  )
}
...
export function ColorPicker({ value, presets = [], fallback, onColor, maxPresets = 8 }) {
  return (
    <div className="gb-colorpick">
      <ColorInput size="sm" value={value || fallback} onChange={onColor} />
      {presets.length > 0 && <span className="gb-colorpick-divider" aria-hidden="true" />}
      {presets.slice(0, maxPresets).map((c) => (
        <button
          key={c}
          type="button"
          className={`gb-accent-dot${(value || '').toLowerCase() === c.toLowerCase() ? ' is-on' : ''}`}
          style={{ background: c }}
          aria-label={`Use ${c}`}
          onClick={() => onColor(c)}
        />
...
export function BadgeAvatars({ badges }) {
  return (
    <span className="gb-badge-avatars">
      {badges.slice(0, 6).map((b) => (
        <span key={b.id} className="gb-badge-avatar" title={b.name}>
          {b.img ? <img src={b.img} alt="" /> : null}
        </span>
      ))}
      <span className="gb-badge-avatars-label">
...
export function BadgeSelect({
  ...
  return (
    <div className="gb-badgeselect">
      {badges.map((b) => {
        const sel = selectedIds.includes(b.id)
        const off = !sel && disabledIds.includes(b.id)
        return (
          <div key={b.id} className={`gb-bsrow${sel ? ' is-on' : ''}${off ? ' is-disabled' : ''}`}>
            <button
              type="button"
              className="gb-bsrow-pick"
              onClick={() => onToggle(b.id)}
              aria-pressed={sel}
              disabled={off}
              title={off ? `${b.name} — ${disabledHint}` : b.name}
            >
              <span className={`gb-bsrow-check${sel ? ' is-on' : ''}`} aria-hidden="true">
                {sel && <GalleryCheck />}
              </span>
              <span className="gb-bsrow-art">
                {b.img ? (
                  <img src={b.img} alt="" draggable={false} />
                ) : (
                  <span className="gb-bsrow-art-ph" />
                )}
              </span>
              <span className="gb-bsrow-name">{b.name}</span>
            </button>
            {off && <span className="gb-bsrow-used">{disabledHint}</span>}
            {valueMode && sel && (
              <div className="gb-bsrow-val">
                <NumberInput
                  value={values[b.id] ?? 1}
                  min={1}
                  max={100}
...
// Badge ids that earn a reward. Rewards attach to the badge that grants them
// (chosen in the badge editor), so this just reads the badges — used to flag the
// rewarded spaces on the gameboard.
export function rewardedBadgeIds(challenge) {
  const c = challenge || {}
  const ids = new Set()
  if (c.registrationBadge?.reward) ids.add('reg')
  if (c.completionBadge?.reward) ids.add('comp')
  const mark = (list, prefix) =>
    (list || []).forEach((b, i) => {
      if (b?.reward) ids.add(`${prefix}-${i}`)
    })
  mark(c.badges, 'log')
  mark(c.activityBadges, 'act')
  mark(c.pointsBadges, 'pts')
  mark(c.reviewBadges, 'rev')
  return ids
}

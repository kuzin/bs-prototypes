...
// All data is fake.
//
// Forked from the Challenge Creator V2 prototype and narrowed to the Gameboard
// challenge type, so the board can be worked on inside the flow that actually
// produces it: pick the type, fill in details, build the badges the board is
// paved with, attach rewards, lay out the board, set completion.
...
const FAV_KEY = 'gb-fav-badges'
const RECENT_KEY = 'gb-recent-badges'
const _readKeys = (k) => {
  try {
...
export function getSteps() {
  // One type, one role — so the sequence is fixed. There is no Rewards step:
  // a reward attaches to the badge that grants it, in the badge editor. The
  // board is built after badges, once there is something to pave it with;
  // completion comes last, as it does everywhere else in the creator.
  return [
    { id: 'type', name: BASE_STEP_NAMES.type },
    { id: 'template', name: BASE_STEP_NAMES.template },
    { id: 'details', name: BASE_STEP_NAMES.details },
    { id: 'badges', name: BASE_STEP_NAMES.badges },
    { id: 'setup', name: 'Gameboard' },
    { id: 'completion', name: BASE_STEP_NAMES.completion },
  ]
}
...
export function blankChallenge(typeId) {
  ...
  return {
    ...
    details: {
      ...
      // Header art is uploaded — the banner-theme picker is gone from this fork.
      background: { kind: 'upload', name: '' },
      templateBanner: null,
      headerFont: 'Poppins',
...
// ─── Board themes ─────────────────────────────────────────────────────────────
// `track` paints the road + the halo behind each disc; `ink` is the deep color
// the START / HALFWAY / FINISH labels and the reward markers pick up.
...
// Resolve the selected theme to { bgImg | board, track, ink }. A custom theme is
// either an uploaded background or a flat color scheme.
export function resolveGameboardTheme(theme, { custom, uploadedBg } = {}) {
  if (theme === 'custom') {
    if (uploadedBg) return { bgImg: uploadedBg, track: '#ffffff', ink: '#0f172a' }
    return {

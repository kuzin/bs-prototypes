...
export function App() {
  ...
  const onMeterChange = useCallback((m) => {
    setMeter((prev) => {
      const next = { ...prev, ...m }
      const same = ['onLoad', 'fired', 'total', 'fullyLoaded', 'mode'].every(
        (k) => prev[k] === next[k],
      )
        ...
        <header className="ins-header">
          <div className="ins-header-inner">
            ...
            <p className="ins-context">
              Today every tile is a separate query, so concurrent users back up the database and the
              page fails for everyone. Switch between concepts to compare the trade-offs.
            </p>
          </div>
        </header>
...
function QueryMeter({ meter }) {
  ...
  return (
    <div className={`ins-qmeter${fullyLoaded ? ' ins-qmeter--done' : ' ins-qmeter--loading'}`}>
      ...
      <div className="ins-qmeter-foot">
        {fired} of {total} fired
      </div>
    </div>
  )
}
